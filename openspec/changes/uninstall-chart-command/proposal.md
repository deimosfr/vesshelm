...
3.  **Interactive Selection**: If `<chart_name>` is omitted, the command MUST list all configured charts (formatted as `name (namespace)`) and allow the user to select one interactively.
4.  **Validation**: Error if the chart is not found in `vesshelm.yaml`.
5.  **Confirmation**: Prompt the user for confirmation (e.g., "Are you sure you want to uninstall <name>? [y/N]") before proceeding.
6.  **Action**: Execute `helm uninstall <name> --namespace <namespace>` upon confirmation.
