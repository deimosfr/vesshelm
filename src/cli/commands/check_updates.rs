        ...
        match repo_type {
            Some(RepoType::Helm) => {
                ...
                let chart_ref = if let Some(repo_name) = &chart.repo_name {
                    ...
                    // Fallback to strict "name" if no repo provided?
                    // But our Config validator enforces chart.repo_name exists and is valid
                    // except for local charts where repo_name might be None.
                    // If repo_name is None, it's a local chart (handled below).
                    chart.name.clone()
                };
            ...
            _ => {
                // Git or Local or OCI (assuming OCI not fully supported in search yet or treated same as Helm?)
                // OCI search syntax is different `helm search repo` won't work for OCI usually.
                // Assuming Git/Local for now as per spec "Ignore local/git charts".
...
fn find_and_replace_version(
    ...
    // We must ensure we don't cross into a new chart definition (start of new list item `-` or unindented `name:` if different structure)
    // Actually, `content[start_idx..]` string slice.

    // We need to operate on the original string to get byte ranges for replacement.
    // Let's iterate over `content.match_indices('\n')` or similar, but simplified:
    // We search for `version:` regex in the remaining string.
    ...
    if let Some(v_cap) = version_regex.captures(suffix) {
        ...
        // A robust heuristic: if we see `- name:` or `  name:` that isn't commented out, we probably crossed boundary.
        // For now, looking for `name:` key at start of line is good check.
